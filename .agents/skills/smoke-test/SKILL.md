...
**CRITICAL**: Before proceeding, parse the requested options and validate:

1. **Parse arguments** from the user's `$smoke-test` request
...
This skill requires browser automation. In Codex, use the Browser plugin or another configured browser/MCP tool for navigation, interaction, and screenshots.
...
The original Claude `model` field was not migrated because Codex skills use the current session or agent model.

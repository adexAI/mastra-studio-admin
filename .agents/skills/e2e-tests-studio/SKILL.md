...
description: "REQUIRED when modifying any file in packages/playground-ui or packages/playground. Triggers on React component creation, UI changes, new playground features, and bug fixes affecting studio UI. Generates Playwright E2E tests that validate product behavior, not just UI states."
---
...
Requires browser automation. In Codex, use the Browser plugin or another configured browser/MCP tool for navigation, interaction, and screenshots. If no browser automation tool is available, tell the user to configure one before writing or validating E2E tests.

The original Claude skill pinned `model: claude-opus-4-5`; Codex does not support skill-level model pins, so this skill runs with the current session or agent model.
...
The original Claude `model` field was not migrated because Codex skills use the current session or agent model. The original trigger guidance is included in the skill description.

---
name: "source-command-gh-debug-issue"
description: "Run the migrated source command `gh-debug-issue`."
---

# source-command-gh-debug-issue

Use this skill when the user asks to run the migrated source command `gh-debug-issue`.
...
1. Record the issue number, title, and a 2-3 sentence summary of the problem as your first task (e.g., "Debug issue #<issue-number>: <title> — <brief summary>"). This ensures the issue context survives in your task state even if conversation history is compressed.
2. Create an `ISSUE_SUMMARY-<issue-number>.md` file in the project root with the issue number, title, labels, and a summary of the issue body and key comments. This file is your persistent reference — you can re-read it at any point if you need to recall the issue details.
...
5. Update your `ISSUE_SUMMARY-<issue-number>.md` with your analysis findings

## Stage 2 "Reproduce"

Once you've analyzed the issue:

1. Update the existing `ISSUE_SUMMARY-<issue-number>.md` file in the project root with a summary of what you've analyzed so far. Do not begin to fix the issue; that isn't our goal.
...
6. When creating commits or PRs, reference the issue number and title. If you've lost track of the details, re-read the `ISSUE_SUMMARY-<issue-number>.md` file.
...
Migrated from source command `gh-debug-issue` into a Codex skill. Invoke it as `$source-command-gh-debug-issue` with the issue number in the user request.

---
name: "source-command-gh-fix-lint"
description: "Run the migrated source command `gh-fix-lint`."
---

# source-command-gh-fix-lint

Use this skill when the user asks to run the migrated source command `gh-fix-lint`.
...
The PR argument in the user request can be either:
...
RUN gh pr view <pr-number-or-url> --json headRefName,headRepository,headRepositoryOwner,number,url
...
RUN git fetch origin pull/<pr-number>/head:<branch-name-from-step-1>
...
Migrated from source command `gh-fix-lint` into a Codex skill. Invoke it as `$source-command-gh-fix-lint` with the PR number or URL in the user request.

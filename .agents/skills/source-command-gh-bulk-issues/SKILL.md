---
name: "source-command-gh-bulk-issues"
description: "Run the migrated source command `gh-bulk-issues`."
---

# source-command-gh-bulk-issues

Use this skill when the user asks to run the migrated source command `gh-bulk-issues`.
...
The user request should include a space-separated list of GitHub issue numbers, e.g. `1234 5678 9012`.
...
3. **Spawn an `mc` headless instance** in each worktree with a generous timeout (30 minutes):
   ```
   cd ../$(basename $PWD)-issue-<NUMBER> && pnpx tsx <path-to-mastracode>/src/main.ts --timeout 1800 --prompt '$source-command-gh-debug-issue <NUMBER>'
   ```
...
4. If the instance **completed its work** (code + tests + changeset):
   ...
   - When approved, commit, push, and create a PR using `$source-command-gh-new-pr` conventions:
...
After PRs are created, spawn `mc` instances with `$source-command-gh-pr-comments <PR_NUMBER>` to handle CodeRabbit and reviewer feedback. If an instance times out, restart it with context about which comments still need addressing.
...
If any checks are failing, spawn an `mc` instance in the worktree with `$source-command-gh-fix-ci` to diagnose and fix the failures. If it times out, restart it with context about which checks failed and what was already attempted.
...
Migrated from source command `gh-bulk-issues` into a Codex skill. Invoke it as `$source-command-gh-bulk-issues` with issue numbers in the user request.
